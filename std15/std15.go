...
func (self *Std15) MapChar (screen *ebiten.Image, cx int32, cy int32, c byte) {
  glyph := FONT[c];
  for y:= int32(0); y < CHAR_SY; y++ {
    ...
    for x:= int32(0); x < CHAR_SX; x++ {
      if ((line >> (CHAR_SX-x-1)) & 0x1) == 0x1 {
        ...
        y0 := float64(cy * CHAR_SY + y) * float64(self.cb_unit);
        x1 := float64(self.cb_unit);
        y1 := float64(self.cb_unit);
        ebitenutil.DrawRect(screen,x0,y0,x1,y1,color.White)
      }
    }
  }
}
...
func (self *Std15) PAppletDraw (screen *ebiten.Image) {
  for y := int32(0); y < self.cb_sy; y++ {
    for x := int32(0); x < self.cb_sx; x++ {
      self.MapChar(screen, x, y, self.charBuff[y * self.cb_sx + x])
    }
  }
}
